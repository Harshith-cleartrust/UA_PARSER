...
function cacheRawForField(field, cacheEntry) {
  ...
  if (field === "CPU") {
    return cacheEntry.CPU ?? cacheEntry.hardware_cpu ?? null;
  }
  if (field === "GPU") {
    return cacheEntry.GPU ?? cacheEntry.hardware_gpu ?? null;
  }
  if (field === "SoC") {
    return cacheEntry.SoC ?? cacheEntry.hardware_chipset ?? null;
  }
...
export function normalizeModelCacheEntry(entry) {
  ...
  const normalized = {
    ...
    SoC: entry.SoC ?? entry.hardware_chipset ?? "",
    CPU: entry.CPU ?? entry.hardware_cpu ?? "",
    GPU: entry.GPU ?? entry.hardware_gpu ?? "",
