...
export const PARSER_API_VERSION = "0.3.56";
...
export function buildDetectionResult(input, deviceCatalog, opts = {}) {
  ...
  browser = reconcileAndroidUaBrowser(ua, browser, platform);
  const crawler = detectCrawlerLabeled(ua);
  const deviceType = deviceClassFromSignals(ua, ch);

  const modelConflict = detectModelSourceConflict({ userAgent: ua, clientHints: ch });
  let modelResolve = resolveHardwareModelKey({ userAgent: ua, clientHints: ch });
  if (modelConflict) {
    modelResolve = { key: "", raw: null, source: "none" };
  }
  const deviceRow =
    !skipDatasetHardware && !modelConflict && modelResolve.key
  ...
  let hwModelDisplay = modelConflict
    ? "N/A"
    : modelResolve.raw || "N/A";
  ...
  const properties = [
    ...
    propRow("SoC", "N/A", "none", "unknown"),
    propRow("CPU", "N/A", "none", "unknown"),
    propRow("GPU", "N/A", "none", "unknown"),
  ...
  return {
    ...
    debug: {
      ...
      modelSourceConflict: modelConflict,
